...
impl BlockchainVMRef {
    ...
    pub fn execute_tx_context(&self, tx_context: TxContext) -> anyhow::Result<TxContext> {
        ...
        Ok(tx_context_sh.into_inner())
    }

    pub fn get_contract_instance(&self, tx_context: &TxContext) -> anyhow::Result<Box<dyn Instance>> {
...
fn get_contract_identifier(tx_context: &TxContext) -> anyhow::Result<Vec<u8>> {
    tx_context
        .tx_cache
