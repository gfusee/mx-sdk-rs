...
pub fn execute_current_tx_context_input() -> anyhow::Result<()> {
...
impl BlockchainVMRef {
    ...
    where
        F: FnOnce() -> anyhow::Result<()>,
    {
        state.with_shared(|state_arc| -> anyhow::Result<(TxResult, BlockchainUpdate)> {
            ...
            Ok(tx_context_sh.into_inner().into_results())
        })
    }
        ...
        if state.accounts.contains_key(&async_data.to) {
            ...
            match result {
                Ok(blockchain_updates) => {
                    ...
                    Ok((TxResult::empty(), TxResult::empty()))
                },
    ...
    fn insert_ghost_account(
        ...
        async_data: &AsyncCallTxData,
        state: &mut Shareable<BlockchainState>,
    ) -> anyhow::Result<Result<BlockchainUpdate, TxPanic>> {
        state.with_shared(|state_arc| -> anyhow::Result<Result<BlockchainUpdate, TxPanic>> {
            let tx_cache = TxCache::new(state_arc);
            if let Err(error) = tx_cache.subtract_egld_balance(&async_data.from, &async_data.call_value)? {
                return Ok(Err(error));
            };
            ...
            Ok(Ok(tx_cache.into_blockchain_updates()))
        })
    }
}
